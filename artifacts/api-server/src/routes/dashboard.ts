...
router.get("/dashboard/summary", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (isGoogleAdsConfigured()) {
    try {
      ...
      logger.warn({ err: err.message }, "Google Ads dashboard summary failed, falling back to SQLite");
    }
  }

  // ── Fallback: SQLite local data ──────────────────────────────────────────
  const now = new Date();
  ...
  const campaigns = db.prepare("SELECT budget, roas FROM campaigns WHERE status = 'ativo'").all() as any[];
...
router.get("/dashboard/performance", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (isGoogleAdsConfigured()) {
    try {
      ...
      logger.warn({ err: err.message }, "Google Ads performance failed, falling back to SQLite");
    }
  }
...
router.get("/dashboard/conversions-by-campaign", requireAuth, async (_req, res): Promise<void> => {
  ...
  if (isGoogleAdsConfigured()) {
    try {
      ...
      logger.warn({ err: err.message }, "Google Ads conversions failed, falling back to SQLite");
    }
  }
