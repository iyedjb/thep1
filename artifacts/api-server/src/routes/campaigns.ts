...
router.get("/campaigns", requireAuth, async (_req, res) => {
  const db = getDb();
  try {
    const rows = await db.prepare("SELECT * FROM campaigns ORDER BY id ASC").all() as any[];
        ...
        for (const row of rows) {
          if (row.google_campaign_id) {
            ...
            if (gc) {
              ...
              await db.prepare("UPDATE campaigns SET status = ?, budget = ? WHERE id = ?")
...
router.get("/campaigns/:id", requireAuth, async (req, res) => {
  const db = getDb();
  try {
    const row = await db.prepare("SELECT * FROM campaigns WHERE id = ?").get(Number(req.params.id)) as any;
    if (!row) {
...
router.patch("/campaigns/:id", requireAuth, async (req, res) => {
  ...
  const db = getDb();
  try {
    const existing = await db.prepare("SELECT * FROM campaigns WHERE id = ?").get(Number(req.params.id)) as any;
    if (!existing) {
      res.status(404).json({ error: "Campaign not found" });
      return;
    }
