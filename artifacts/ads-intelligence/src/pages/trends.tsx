...
// Todos os ~193 países reconhecidos pela ONU (+ Vaticano), em ordem alfabética
// (pelo nome em português) — "Global" fica sempre como primeira opção, fora
// da ordenação, por ser o padrão "sem país específico".
...
function GoogleTrendsWidget({ keyword, geo, timeRange, type }: WidgetProps) {
  ...
  useEffect(() => {
    ...
    const timeCodeRaw = timeRange === "12m" ? "today 12-m" :
                        timeRange === "90d" ? "today 3-m" :
                        timeRange === "30d" ? "today 1-m" :
                        timeRange === "7d" ? "now 7-d" :
                        timeRange === "1d" ? "now 1-d" :
                        timeRange === "4h" ? "now 4-H" :
                        timeRange;
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Dados reais e inteligência geográfica de tendências de pesquisa</p>
        </div>
      </div>
