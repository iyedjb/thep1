...
type AdminRole = { key: string; name: string; description: string; permissions: Permission[] };
type AdminInvitation = { id: number; email: string; roleKey: string; roleName: string; permissions: Permission[]; expiresAt: string; createdAt: string };
...
const actionLabels: Record<string, string> = {
  "admin.login": "Entrou no painel", "admin.owner.created": "Criou o acesso proprietário", "admin.access.invited": "Enviou um convite administrativo", "admin.access.accepted": "Aceitou um convite administrativo", "admin.access.created": "Criou um acesso administrativo", "admin.access.status.updated": "Alterou o status de um acesso", "admin.access.deleted": "Excluiu um acesso administrativo",
...
export default function AdminPage() {
  ...
  const [adminForm, setAdminForm] = useState({ email: "", roleKey: "support" });
  ...
  const loadSection = async (target: Section) => {
    ...
    try {
      ...
      if (target === "access") {
        const [accessData, roleData] = await Promise.all([adminApi("/api/admin/accounts"), adminApi("/api/admin/roles")]);
        setAccounts(accessData.accounts || []);
  ...
  const createAdmin = async (event: FormEvent) => {
    event.preventDefault();
    try { await adminApi("/api/admin/accounts", { method: "POST", body: JSON.stringify(adminForm) }); setCreateAdminOpen(false); setAdminForm({ email: "", roleKey: "support" }); await loadSection("access"); toast({ title: "Convite enviado", description: "O administrador definirá o próprio nome e senha pelo link recebido." }); }
    catch (error: any) { toast({ title: "Não foi possível enviar o convite", description: error.message, variant: "destructive" }); }
  };
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="lg:pl-64">
        ...
        <div className="mx-auto max-w-7xl px-5 py-8 lg:px-10 lg:py-12">
          ...
          {!loading && section === "access" && <section>
            <div className="flex items-start justify-between gap-5"><div><h2 className="text-2xl font-semibold tracking-tight">Equipe administrativa</h2><p className="mt-2 text-sm text-muted-foreground">Envie um convite e escolha um papel com permissões realmente aplicadas no servidor.</p></div><Button onClick={() => setCreateAdminOpen(true)} className="rounded-full px-6">Convidar administrador</Button></div>
            {invitations.length > 0 && <div className="mt-8"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Convites pendentes</p><div className="mt-3 divide-y divide-border border-y border-border">{invitations.map((invitation) => <div key={invitation.id} className="flex items-center gap-4 py-4"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-amber-50 text-sm font-semibold text-amber-700">@</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{invitation.email}</p><p className="mt-1 text-xs text-muted-foreground">{invitation.roleName} · aguardando aceite</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-700">Pendente</span></div>)}</div></div>}
      ...
      <Dialog open={createAdminOpen} onOpenChange={setCreateAdminOpen}><DialogContent className="max-w-lg rounded-3xl p-7"><DialogTitle className="text-2xl">Convidar administrador</DialogTitle><DialogDescription>Informe somente o e-mail e escolha o papel. A pessoa criará o próprio nome e senha pelo link seguro.</DialogDescription><form onSubmit={createAdmin} className="mt-6 space-y-4"><Input type="email" placeholder="E-mail do administrador" value={adminForm.email} onChange={(event) => setAdminForm({ ...adminForm, email: event.target.value })} className="h-11 rounded-2xl" required/><Select value={adminForm.roleKey} onValueChange={(value) => setAdminForm({ ...adminForm, roleKey: value })}><SelectTrigger className="h-11 rounded-2xl"><SelectValue placeholder="Selecione o papel"/></SelectTrigger><SelectContent>{roles.map((role) => <SelectItem key={role.key} value={role.key}>{role.name}</SelectItem>)}</SelectContent></Select>{roles.find((role) => role.key === adminForm.roleKey) ? <div className="rounded-2xl border border-border bg-muted/20 p-4"><p className="text-sm font-semibold">{roles.find((role) => role.key === adminForm.roleKey)?.name}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{roles.find((role) => role.key === adminForm.roleKey)?.description}</p><div className="mt-3 flex flex-wrap gap-2">{roles.find((role) => role.key === adminForm.roleKey)?.permissions.map((permission) => <span key={permission} className="rounded-full bg-background px-3 py-1 text-[11px] text-muted-foreground">{permissionLabels.find((item) => item.id === permission)?.label || permission}</span>)}</div></div> : null}<Button type="submit" className="h-11 w-full rounded-full">Enviar convite</Button></form></DialogContent></Dialog>
